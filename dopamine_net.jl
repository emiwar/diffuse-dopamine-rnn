...
function DOPAMINE_NET(W_in, W_rec, W_out, DA, tau_m, eta_in, eta_rec, eta_out)
    n_hidden, n_in = size(W_in)
    DOPAMINE_NET(W_in, W_rec, W_out, DA, tau_m, eta_in, eta_rec, eta_out,
             zeros(n_hidden), zeros(n_hidden), zero(W_rec), zero(W_in))
end

function DOPAMINE_NET(W_in, W_rec, W_out, DA, tau_m, eta)
    DOPAMINE_NET(W_in, W_rec, W_out, DA, tau_m, eta, eta, eta)
end

function DOPAMINE_NET(n_in, n_hidden, n_out; in_scale=0.8, rec_scale=-2.0,
    ...
    DA = draw_DA(n_hidden, n_out; lambda=lambda)
    DA[end÷2+1:end, :] .*= -1
    DOPAMINE_NET(W_in, W_rec, W_out, DA, tau_m, eta_in, eta_rec, eta_out)
end
function reset_state!(net::DOPAMINE_NET)
    net.v .= 0.0
    net.p .= 0.0
    net.q .= 0.0
    
end
function step!(net::DOPAMINE_NET, inp, target)
...
function episode!(net::DOPAMINE_NET, inp, target)
    reset_state!(net)
    t_max = size(inp)[1]
    y = zero(target)
...
function draw_DA(n_hidden, n_outputs; n_varicosities=10, lambda=0.1)
    DA = zeros(n_hidden, n_outputs)
    hidden_pos = rand(n_hidden, 3)
    for i=1:n_outputs
        varicosities_pos = rand(n_varicosities, 3)
        for j=1:n_hidden, k=1:n_varicosities
            d = norm(hidden_pos[j, :] - varicosities_pos[k, :])
            DA[j, i] += exp(-d/lambda)
        end
    end
    return DA
