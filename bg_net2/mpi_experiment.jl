...
const DEFAULT_PARAMS = (net_size=200, trial_length=200,
                        ...
                        dopamine_tau=50.0, repetition=1)
...
function train_network(desc::String; net_size=200, trial_length=200,
                        ...
                        synapseType=EligabilitySynapse, n_varicosities=10,
                        dopamine_tau=50.0, repetition=1)
    net = BgNet(net_size, target_dim, learning_rate, learning_rate*feedback_factor, lambda=lambda, SynapseType=synapseType, n_varicosities=n_varicosities, dopamine_tau=dopamine_tau)
...
function run_experiment(fn; params...)
    ...
    comm_size::Int64 = MPI.Comm_size(comm)
    comm_rank::Int64 = MPI.Comm_rank(MPI.COMM_WORLD)
    for (i, values) in enumerate(product(param_ranges...))
    	if (i-1)%comm_size != comm_rank
	    continue
	end
        ...
        net, losses = train_network("$named_params "; all_params...) #,angles
        h5open("$(fn)_$(i).h5", "cw") do fid
            fid["run$i"] = losses
...
println("Process $(MPI.Comm_rank(comm)) of $(MPI.Comm_size(comm)) ready.")
MPI.Barrier(MPI.COMM_WORLD)
run_experiment("data/test_smoothed_dopamine",
               striatumUpdate=[:smoothed_dopamine, :dopamine, :no_dopamine],
	       dopamine_tau=(10.0 .^ (-1:0.2:4)),
               repetition=1:25)
MPI.Finalize()
